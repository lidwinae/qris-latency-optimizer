...
func TestTransactionUsecaseConfirmPaymentSyncUpdatesCacheAndNotification(t *testing.T) {
	merchant := &entity.Merchant{ID: uuid.New(), QRID: "TEST001", MerchantName: "Kantin", IsActive: true}
	...
	resp, err := u.ConfirmPaymentSync(txID.String())
	if err != nil {
		t.Fatalf("ConfirmPaymentSync returned error: %v", err)
	}
